...
@section('content')
  ...
  @php
    ...
    // 1) Ambil dari variabel yang mungkin dikirim controller
    $src = $tenants ?? $penghuni ?? $items ?? $data ?? $list ?? null;

    $paginator = null;
    if ($src instanceof PagiContract) {
        $paginator = $src;
        $rows = collect($src->items());
    } elseif (is_iterable($src)) {
        $rows = collect($src);
    } else {
        // 2) FALLBACK: kalau controller tidak mengirim data, ambil dari DB
        $src = \App\Models\Tenant::with(['user:id,name,email,phone','room:id,kode,nama'])
                ->latest()->paginate(12);
        $paginator = $src;
        $rows = collect($src->items());
    }
      ...
      <tbody>
        @forelse($rows as $t)
          @php
            ...
            $hp     = $t->user->phone   ?? $t->phone ?? $t->no_hp ?? '-';
            $nik    = $t->nik           ?? '-';
